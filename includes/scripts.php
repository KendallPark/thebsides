...
function registerScripts() {
	...
	if(!is_admin()) {
		 ...
		 */
		wp_register_script('modernizer', get_bloginfo('template_directory') .'/js/modernizr.js','jquery'); // Always include Modernizr when using HTML5 * You should always redownload a minified/minimal version of Modernizr when pushing to a live site @ http://modernizr.com/download/
		wp_enqueue_script('modernizer'); // Load Modernizr into the header site wide 
		
		//wp_register_script( 'ui', get_bloginfo('template_directory') .'/js/jquery-ui-1.8.18.custom.min.js','jquery'); // Load only if you need to use the jQuery UI library.  You will need to upload your own custom script to the themes js folder
